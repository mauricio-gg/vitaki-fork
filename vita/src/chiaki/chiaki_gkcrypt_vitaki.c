...
#include "chiaki_gkcrypt_vitaki.h"
...
CHIAKI_EXPORT ChiakiErrorCode chiaki_gkcrypt_init(
  ...
  if (gkcrypt->key_buf_size) {
    ...
    if (err != CHIAKI_ERR_SUCCESS) goto error_key_buf;

    err = chiaki_cond_init(&gkcrypt->key_buf_cond, &gkcrypt->key_buf_mutex);
    if (err != CHIAKI_ERR_SUCCESS) goto error_key_buf_mutex;
  } else {
  ...
  if (gkcrypt->key_buf) {
    err = chiaki_thread_create(&gkcrypt->key_buf_thread, gkcrypt_thread_func,
                               gkcrypt);
    if (err != CHIAKI_ERR_SUCCESS) goto error_key_buf_cond;
...
error_key_buf_cond:
  if (gkcrypt->key_buf) chiaki_cond_fini(&gkcrypt->key_buf_cond);
error_key_buf_mutex:
  if (gkcrypt->key_buf) chiaki_mutex_fini(&gkcrypt->key_buf_mutex);
error_key_buf:
...
CHIAKI_EXPORT ChiakiErrorCode chiaki_gkcrypt_gen_key_stream(
  ...
  if (!ctx) return CHIAKI_ERR_UNKNOWN;
...
CHIAKI_EXPORT ChiakiErrorCode chiaki_gkcrypt_get_key_stream(
  ...
  if (signal) chiaki_cond_signal(&gkcrypt->key_buf_cond);
...
CHIAKI_EXPORT ChiakiErrorCode chiaki_gkcrypt_decrypt(ChiakiGKCrypt *gkcrypt,
  ...
  if (!key_stream) return CHIAKI_ERR_MEMORY;
...
CHIAKI_EXPORT ChiakiErrorCode chiaki_gkcrypt_gmac(ChiakiGKCrypt *gkcrypt,
  ...
  if (mbedtls_gcm_starts(&actx, MBEDTLS_GCM_ENCRYPT, iv,
                         CHIAKI_GKCRYPT_BLOCK_SIZE, NULL, 0) != 0) {
    mbedtls_gcm_free(&actx);
    return CHIAKI_ERR_UNKNOWN;
  }
  ...
  if (mbedtls_gcm_crypt_and_tag(
          &actx, MBEDTLS_GCM_ENCRYPT, 0, iv, CHIAKI_GKCRYPT_BLOCK_SIZE, buf,
          buf_size, NULL, NULL, CHIAKI_GKCRYPT_GMAC_SIZE, gmac_out) != 0) {
    mbedtls_gcm_free(&actx);
    return CHIAKI_ERR_UNKNOWN;
  }
...
static bool key_buf_mutex_pred(void *user) {
  ...
  if (gkcrypt->key_buf_thread_stop) return true;

  if (gkcrypt->key_buf_populated < gkcrypt->key_buf_size) return true;

  if (gkcrypt_key_buf_should_generate(gkcrypt)) return true;
...
static void *gkcrypt_thread_func(void *user) {
  ...
  while (1) {
    ...
    if (gkcrypt->key_buf_thread_stop || err != CHIAKI_ERR_SUCCESS) break;
