...
typedef struct chiaki_gkcrypt_t {
  ...
  size_t key_buf_size;
  size_t key_buf_populated;  // size of key_buf that is already populated (on
                             // startup)
...
/**
 * @param key_buf_chunks if > 0, use a thread to generate the ctr mode key
 * stream
 */
...
CHIAKI_EXPORT ChiakiErrorCode chiaki_gkcrypt_decrypt(ChiakiGKCrypt *gkcrypt,
                                                     ...
                                                     uint8_t *buf,
                                                     size_t buf_size);
static inline ChiakiErrorCode chiaki_gkcrypt_encrypt(ChiakiGKCrypt *gkcrypt,
                                                     uint64_t key_pos,
                                                     uint8_t *buf,
                                                     size_t buf_size) {
  return chiaki_gkcrypt_decrypt(gkcrypt, key_pos, buf, buf_size);
}
...
static inline ChiakiGKCrypt *chiaki_gkcrypt_new(ChiakiLog *log,
                                                size_t key_buf_chunks,
                                                uint8_t index,
                                                const uint8_t *handshake_key,
                                                const uint8_t *ecdh_secret) {
  ChiakiGKCrypt *gkcrypt = CHIAKI_NEW(ChiakiGKCrypt);
  if (!gkcrypt) return NULL;
  ChiakiErrorCode err = chiaki_gkcrypt_init(gkcrypt, log, key_buf_chunks, index,
                                            handshake_key, ecdh_secret);
...
/**
 * @param commit whether to remember this key_pos to update the state. Should
 * only be true after authentication to avoid DoS.
 */
