...
#define CHECK(err)             \
  ...
  // build key
  CHECK(mbedtls_ecdh_gen_public(&ecdh->ctx.grp, &ecdh->ctx.d, &ecdh->ctx.Q,
...
CHIAKI_EXPORT ChiakiErrorCode chiaki_ecdh_set_local_key(
    ChiakiECDH *ecdh, const uint8_t *private_key, size_t private_key_size,
  ...
  // public
  int r = 0;
  r = mbedtls_ecp_point_read_binary(&ecdh->ctx.grp, &ecdh->ctx.Q, public_key,
                                    public_key_size);
  ...
  // regen key
  r = mbedtls_ecdh_gen_public(&ecdh->ctx.grp, &ecdh->ctx.d, &ecdh->ctx.Q,
                              mbedtls_ctr_drbg_random, &ecdh->drbg);
  if (r != 0) return CHIAKI_ERR_UNKNOWN;
  ...
  if (!private_key_bn) return CHIAKI_ERR_UNKNOWN;
  ...
  if (!EC_POINT_oct2point(ecdh->group, public_key_point, public_key,
...
CHIAKI_EXPORT ChiakiErrorCode chiaki_ecdh_get_local_pub_key(
    ChiakiECDH *ecdh, uint8_t *key_out, size_t *key_out_size,
    const uint8_t *handshake_key, uint8_t *sig_out, size_t *sig_out_size) {
#ifdef CHIAKI_LIB_ENABLE_MBEDTLS
  mbedtls_md_context_t ctx;
  mbedtls_md_init(&ctx);

#define GOTO_ERROR(err) \
  do {                  \
    if ((err) != 0) {   \
      goto error;       \
    }                   \
  ...
  GOTO_ERROR(mbedtls_ecp_point_write_binary(
      &ecdh->ctx.grp, &ecdh->ctx.Q, MBEDTLS_ECP_PF_UNCOMPRESSED, key_out_size,
  ...
  GOTO_ERROR(
      mbedtls_md_hmac_starts(&ctx, handshake_key, CHIAKI_HANDSHAKE_KEY_SIZE));
...
error:
  ...
  if (!point) return CHIAKI_ERR_UNKNOWN;

  *key_out_size =
      EC_POINT_point2oct(ecdh->group, point, POINT_CONVERSION_UNCOMPRESSED,
                         key_out, *key_out_size, NULL);
  if (!(*key_out_size)) return CHIAKI_ERR_UNKNOWN;

  if (!HMAC(EVP_sha256(), handshake_key, CHIAKI_HANDSHAKE_KEY_SIZE, key_out,
            *key_out_size, sig_out, (unsigned int *)sig_out_size))
    return CHIAKI_ERR_UNKNOWN;
  return CHIAKI_ERR_SUCCESS;
...
CHIAKI_EXPORT ChiakiErrorCode chiaki_ecdh_derive_secret(
    ChiakiECDH *ecdh, uint8_t *secret_out, const uint8_t *remote_key,
    size_t remote_key_size, const uint8_t *handshake_key,
    const uint8_t *remote_sig, size_t remote_sig_size) {
...
#define GOTO_ERROR(err) \
  ...
  GOTO_ERROR(mbedtls_ecp_point_read_binary(&ecdh->ctx.grp, &ecdh->ctx.Qp,
                                           remote_key, remote_key_size));

  // build shared secret (diffie-hellman)
  GOTO_ERROR(mbedtls_ecdh_compute_shared(&ecdh->ctx.grp, &ecdh->ctx.z,
                                         &ecdh->ctx.Qp, &ecdh->ctx.d,
                                         mbedtls_ctr_drbg_random, &ecdh->drbg));

  // export shared secret to data buffer
  GOTO_ERROR(mbedtls_mpi_write_binary(&ecdh->ctx.z, secret_out,
...
error:
  ...
  if (!remote_public_key) return CHIAKI_ERR_UNKNOWN;

  if (!EC_POINT_oct2point(ecdh->group, remote_public_key, remote_key,
                          remote_key_size, NULL)) {
    EC_POINT_free(remote_public_key);
    return CHIAKI_ERR_UNKNOWN;
  }

  int r = ECDH_compute_key(secret_out, CHIAKI_ECDH_SECRET_SIZE,
                           remote_public_key, ecdh->key_local, NULL);

  EC_POINT_free(remote_public_key);

  if (r != CHIAKI_ECDH_SECRET_SIZE) return CHIAKI_ERR_UNKNOWN;
