...
CHIAKI_EXPORT ChiakiErrorCode chiaki_http_header_parse(
    ChiakiHttpHeader **header, char *buf, size_t buf_size) {
  *header = NULL;
#define FAIL(reason)                  \
  do {                                \
    chiaki_http_header_free(*header); \
    return (reason);                  \
  } while (0);
  char *key_ptr = buf;
  char *value_ptr = NULL;
  ...
  for (char *end = buf + buf_size; buf < end; buf++) {
    ...
    if (!value_ptr) {
      if (c == ':') {
        if (key_ptr == buf) FAIL(CHIAKI_ERR_INVALID_DATA);
        *buf = '\0';
        buf++;
        if (buf == end) FAIL(CHIAKI_ERR_INVALID_DATA);
        if (*buf == ' ') buf++;
        if (buf == end) FAIL(CHIAKI_ERR_INVALID_DATA);
        value_ptr = buf;
      } else if (c == '\r' || c == '\n') {
      ...
      if (c == '\r' || c == '\n') {
        ...
        *buf = '\0';
        ...
        if (!entry) FAIL(CHIAKI_ERR_MEMORY);
        entry->key = key_ptr;
...
CHIAKI_EXPORT ChiakiErrorCode chiaki_http_response_parse(
    ChiakiHttpResponse *response, char *buf, size_t buf_size) {
  static const char *http_version = "HTTP/1.1 ";
  static const size_t http_version_size = 9;

  if (buf_size < http_version_size) return CHIAKI_ERR_INVALID_DATA;
  ...
  if (!line_end) return CHIAKI_ERR_INVALID_DATA;
  size_t line_length = (line_end - buf) + 1;
  if (buf_size <= line_length) return CHIAKI_ERR_INVALID_DATA;
  ...
  if (response->code == 0) return CHIAKI_ERR_INVALID_DATA;
...
CHIAKI_EXPORT ChiakiErrorCode chiaki_recv_http_header(
    int sock, char *buf, size_t buf_size, size_t *header_size,
    size_t *received_size, ChiakiStopPipe *stop_pipe, uint64_t timeout_ms) {
  // 0 = ""
  // 1 = "\r"
  // 2 = "\r\n"
  // 3 = "\r\n\r"
  ...
  *received_size = 0;
  while (true) {
    if (stop_pipe) {
      ChiakiErrorCode err =
          chiaki_stop_pipe_select_single(stop_pipe, sock, false, timeout_ms);
...
// PSN function disabled for VitaRPS5 integration
/*
CHIAKI_EXPORT ChiakiErrorCode chiaki_send_recv_http_header_psn(ChiakiRudp rudp,
ChiakiLog *log, uint16_t *remote_counter, char *send_buf, size_t send_buf_size,
        char *buf, size_t buf_size, size_t *header_size, size_t *received_size)
{
        ...
        *received_size = 0;
        ...
        RudpMessage message;
        ChiakiErrorCode err;
        err = chiaki_rudp_send_recv(rudp, &message, (uint8_t *)send_buf,
send_buf_size, *remote_counter, SESSION_MESSAGE, CTRL_MESSAGE, 2, 3); if(err !=
CHIAKI_ERR_SUCCESS)
        {
                CHIAKI_LOGE(log, "Didn't receive http session message
response"); return err;
        }
        ...
        *remote_counter = message.remote_counter;
        ...
        if(received <= 0)
                return received == 0 ? CHIAKI_ERR_DISCONNECTED :
CHIAKI_ERR_NETWORK;
