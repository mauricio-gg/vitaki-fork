...
CHIAKI_EXPORT ChiakiErrorCode chiaki_base64_encode(const uint8_t *in,
                                                   size_t in_size, char *out,
                                                   size_t out_size) {
  ...
  for (x = 0; x < in_size; x += 3) {
    ...
    if ((x + 1) < in_size) n += ((uint32_t)in[x + 1]) << 8;

    if ((x + 2) < in_size) n += in[x + 2];
    ...
    if (result_index >= out_size) return CHIAKI_ERR_BUF_TOO_SMALL;
...
CHIAKI_EXPORT ChiakiErrorCode chiaki_base64_decode(const char *in,
  ...
  while (in < end) {
    ...
    switch (c) {
      ...
      default:
        ...
        if (iter == 4) {
          if ((len += 3) > *out_size) return CHIAKI_ERR_BUF_TOO_SMALL;
  ...
  if (iter == 3) {
    if ((len += 2) > *out_size) return CHIAKI_ERR_BUF_TOO_SMALL;
    *(out++) = (unsigned char)((buf >> 10) & 0xff);
    *(out++) = (unsigned char)((buf >> 2) & 0xff);
  } else if (iter == 2) {
    if (++len > *out_size) return CHIAKI_ERR_BUF_TOO_SMALL;
