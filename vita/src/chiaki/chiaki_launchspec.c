...
// VitaRPS5 adaptation of vitaki-fork launchspec.c
...
int chiaki_launchspec_format(char *buf, size_t buf_size,
  ...
  VitaRPS5Result err = chiaki_base64_encode(
      launch_spec->handshake_key, CHIAKI_HANDSHAKE_KEY_SIZE, handshake_key_b64,
      sizeof(handshake_key_b64));
  if (err != VITARPS5_SUCCESS) return -1;

  char *extras[3];
  if (chiaki_target_is_ps5(
          launch_spec->target))  // TODO: probably also for ps4, but only 12
  {
  ...
  int written = snprintf(
      buf, buf_size, launchspec_fmt, launch_spec->width, launch_spec->height,
      launch_spec->max_fps, launch_spec->bw_kbps_sent, launch_spec->mtu,
      launch_spec->rtt, extras[0], extras[1], extras[2], handshake_key_b64);
